...
* | -          | voice_fallback | int            | -     | 1             | 0        | It enables/disables the automatic setting of the voice name to the first available one if the selected voice is not valid                                                                                                                      | -     |
*
* The device can be launched by yarpdev using one of the following examples (with and without all optional parameters):
* \code{.unparsed}
* yarpdev --device googleSpeechSynthesizer --language_code <mandatory_value> --voice_name <optional_value> --voice_speed 1 --voice_pitch 0 --default_voices <optional_value> --voice_fallback 1
* \endcode
*
* \code{.unparsed}
...
public:
    ...
    const std::string m_voice_fallback_defaultValue = {"1"};
